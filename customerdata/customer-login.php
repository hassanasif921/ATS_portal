...
if(isset($_POST["btn_login"]))
{
	...
	mysqli_select_db($connection,"ats-database");
	$user_name = $_POST["username"];
	$user_password = $_POST["password"];

	$query = mysqli_query($connection,"select * from ats_customer where ats_customer_username ='".$user_name."'");
	$row = mysqli_fetch_array($query);
	
	if($row ["ats_customer_username"] == $user_name && $row ["ats_customer_password"] == $user_password)
	{
        $_SESSION["userName"] = $row["ats_customer_username"];
        $_SESSION["customer"] = $row["ats_customer_id"];
		header("Location:customer_mypage.php");
	}
	else
	{
                                ...
                                <div>
                                    <div class="position-relative h-100 d-flex justify-content-center align-items-center bg-plum-plate" tabindex="-1">
                                        ...
                                        <div class="slider-content">
                                            <h3>Japanese Best Quality Used Cars</h3>
                                            <p>To Deliver SERVICE <br/>
                                            You Need Knowledge.</br>
                                            To Deliver GOOD SERVICE </br>
                                            You Have To Add Understanding.</br>
                                            But Delivering EXCELLENT SERVICE </br>
                                            Requires Passion.</br>
                                            </p>
                                        </div>
                                    </div>
