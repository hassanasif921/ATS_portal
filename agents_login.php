...
if(isset($_POST["btn_login"]))
{
	...
	$query = mysqli_query($connection,"select * from ats_employee where username ='".$user_name."'");
	$row = mysqli_fetch_array($query);
	
	if($row ["username"] == $user_name && $row ["password"] == $user_password)
	{
        $_SESSION['agents_name'] = $row['admin_username'];
        $_SESSION['agents_id'] = $row['ats_employee_id'];
		header("Location:index.php");
	}
	else
	{
